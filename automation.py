...
class automation:
    def __init__(self, car, polygon, screen, carSurface):
        ...
        self.screenMask = self.createMask(
            screen, color=(255, 255, 255, 255), treshold=(10, 10, 10, 10)
        )
    ...
    def findPath(self, start_pos, end_pos):
        ...
        start_x, start_y = int(start_pos[0] / self.grid_resolution), int(
            start_pos[1] / self.grid_resolution
        )
        end_x, end_y = int(end_pos[0] / self.grid_resolution), int(
            end_pos[1] / self.grid_resolution
        )
        ...
        if (
            start_x < 0
            or start_x >= len(self.matrix[0])
            or start_y < 0
            or start_y >= len(self.matrix)
        ):
            return []
        if (
            end_x < 0
            or end_x >= len(self.matrix[0])
            or end_y < 0
            or end_y >= len(self.matrix)
        ):
            return []
    ...
    def checkParkingSpot(self, screen, rect):
        ...
        if overlapMask.count() > 0:  # Only draw if there is an overlap
            ...
            try:
                ...
                overlap_viz_surface = overlapMask.to_surface(
                    setcolor=(0, 255, 0, 180),  # Semi-transparent green
                    unsetcolor=(0, 0, 0, 0),
                )
    ...
    def park(self, spaceCenter):
        if spaceCenter:
            ...
            if self.parkingSpaceRect:
                ...
                self.parkingSpaceRect = pygame.Rect(
                    self.x,
                    self.y,
                    self.polygon.parkingSpaceWidth,
                    self.polygon.parkingSpaceLength,
                )
