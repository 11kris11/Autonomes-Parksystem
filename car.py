...
class Car:
    ...
    def __init__(self, x, y, direction, screen, color, color2):
        ...
        self.axleLen = (
            135  # Achsabstand für das Einspurmodell (beeinflusst den Wendekreis)
        )
    ...
    def draw_parked_car(self, screen, einspur=False):
        ...
        roof = pygame.Rect(
            self.x + 50, self.y + 12.5, self.car_roof_length, self.car_roof_width
        )
        ...
        # Lichter
        light1 = pygame.Rect(
            self.x + self.car_length - 20, self.y + self.car_width - 20, 20, 20
        )
        ...
        if einspur:
            ...
            pygame.draw.rect(
                ...
                "black",
                (
                    self.rearTireCenter[0],
                    self.rearTireCenter[1] - 5,
                    self.frontTireCenter[0] - self.rearTireCenter[0],
                    10,
                ),
            )
            ...
            screen.blit(
                self.rearTire,
                (
                    self.rearTireCenter[0] - self.frontTire.get_width() / 2,
                    self.rearTireCenter[1] - self.frontTire.get_height() / 2,
                ),
            )
    ...
    def move_forward(self):  # Wenn (self.vel + self.acceleration) größer als
        self.vel = min(
            self.vel + self.acceleration, self.max_vel
        )  # max_vel, dann max_vel sonst nicht
        self.move()
